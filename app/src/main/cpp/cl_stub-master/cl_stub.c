...
static int open_libopencl_so() {
    ...
    if (path) {
        ...
        if (!so_handle) {  // wil faile from android N
            char *err = dlerror();
            LOGI("\nopen OpenCL library error: %s\n", err);
        }
    }

#if defined(__ANDROID__) || defined(ANDROID)
    if (!so_handle) { //try  local so
        pid_t pid = getpid();
...
//            LOGI("\n code_path %ld %s\n", strlen(code_path), code_path);
            ...
            memcpy(real_code_path, &code_path[prefix_len],
                   strlen(code_path) - prefix_len - suffix_len);
            real_code_path[strlen(code_path) - prefix_len - suffix_len] = '\0';

//            LOGI("\n real_code_path %ld %s\n", strlen(real_code_path), real_code_path);
            path = (char *) malloc(strlen(real_code_path) + 50 );
